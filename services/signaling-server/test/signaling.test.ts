...
describe("session signaling", () => {
  it("answers health checks and rejects malformed session requests", async () => {
    ...
    const malformed = await exports.default.fetch(
      "https://example.test/sessions/SHORT",
    );
    expect(malformed.status).toBe(400);

    const wrongMethod = await exports.default.fetch(
      "https://example.test/sessions/ABC999",
      {
        method: "POST",
      },
    );
  ...
  it("rate limits repeated host creation before Durable Object routing", async () => {
    ...
    const blocked = await exports.default.fetch(
      "https://example.test/sessions/ABC227/host",
      {
        headers,
      },
    );
  ...
  it("rejects duplicate hosts and clients", async () => {
    ...
    const duplicateHost = await exports.default.fetch(
      "https://example.test/sessions/DEF999/host",
      {
  ...
  it("relays answers and ICE candidates, then closes a peer that exceeds its signaling quota", async () => {
    ...
    client.send(
      JSON.stringify({ type: "answer", payload: { sdp: "test-answer" } }),
    );
    ...
    for (let message = 0; message < 30; message += 1) {
      host.send(
        JSON.stringify({ type: "ice_candidate", payload: { message } }),
      );
    }
