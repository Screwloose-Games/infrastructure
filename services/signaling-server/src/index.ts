 ...
 */
function anonymousRequesterKey(request: Request, endpoint: SessionEndpoint): string {
...
async function enforceRequestRateLimit(
  ...
  const limiter = isHostCreation ? env.HOST_CREATION_LIMIT : env.SESSION_REQUEST_LIMIT
 ...
 */
function parseSignalingMessage(rawMessage: string | ArrayBuffer): SignalingMessage | null {
  ...
  if (decoded.type !== 'offer' && decoded.type !== 'answer' && decoded.type !== 'ice_candidate') {
    return null
  }
...
export class SignalSession extends DurableObject<Env> {
  ...
  private socketForRole(role: ConnectionRole): WebSocket | null {
    const socket = this.ctx.getWebSockets().find((candidate) => {
      const attachment = candidate.deserializeAttachment() as ConnectionAttachment | null
   ...
   */
  private consumeSignalingQuota(socket: WebSocket, attachment: ConnectionAttachment): boolean {
    const now = Date.now()
    const windowExpired = now - attachment.signalWindowStartedAt >= SIGNALING_WINDOW_MS
  ...
  override async fetch(request: Request): Promise<Response> {
    ...
    if (route.endpoint === 'join_socket' && !this.hostIsConnected()) {
      return Response.json({ error: 'No live host exists for this session.' }, { status: 409 })
    }
    ...
    const role: ConnectionRole = route.endpoint === 'host_socket' ? 'host' : 'client'
  ...
  override async webSocketMessage(
    ...
    if (!this.consumeSignalingQuota(socket, sender)) {
      console.warn(JSON.stringify({ event: 'signal_rate_limited', role: sender.role }))
      this.rejectAndClose(socket, 'Too many signaling messages. Please reconnect and try again.')
      return
    }
    ...
    if (message === null) {
      ...
      this.rejectAndClose(socket, 'Expected a small JSON offer, answer, or ice_candidate message.')
      return
    }
    ...
    if (sender.role === 'host' && message.type === 'answer') {
      ...
      this.rejectAndClose(socket, 'Only the joining client may send an answer.')
      return
    }
    ...
    const recipientRole: ConnectionRole = sender.role === 'host' ? 'client' : 'host'
  ...
  override async webSocketClose(
    ...
    const remainingRole: ConnectionRole = departed.role === 'host' ? 'client' : 'host'
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const rateLimitResponse = await enforceRequestRateLimit(request, route, env)
